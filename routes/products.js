...
router.get("/view/product/:id", cacheMiddleware("products"), async (req, res) => {
  try {
    ...
    // Step 8: Render the product page with the adjusted price and coupons
    res.render("view-product", { product: sanitizeProduct(product), currency: userCurrency, coupons: productCoupons });
  } catch (error) {
